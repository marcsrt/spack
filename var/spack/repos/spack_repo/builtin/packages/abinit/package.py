...
class Abinit(AutotoolsPackage, CudaPackage, ROCmPackage):
    ...
    variant("gpu_markers", when="@10.4: +gpu_openmp_offload", default=False, description="Enable GPU markers, either NVTX for CUDA or rocTX for ROCM")
    ...
    conflicts("+gpu_openmp_offload",    when="@:10.2",  msg="ABINIT builds with GPU support are only supported from v10.4")
    conflicts("+gpu_openmp_offload",    when="~mpi",    msg="ABINIT builds with GPU support require MPI support enabled as well")
    conflicts("+cuda",              when="~gpu_openmp_offload",    msg="ABINIT builds with GPU support require OpenMP offload support enabled as well")
    conflicts("+rocm",              when="~gpu_openmp_offload",    msg="ABINIT builds with GPU support require OpenMP offload support enabled as well")
    ...
    for arch in ROCmPackage.amdgpu_targets:
        depends_on(
            "hipblas+rocm amdgpu_target=%s" % arch, when="+rocm amdgpu_target=%s" % arch
        )
        depends_on(
            "hipfft+rocm amdgpu_target=%s" % arch, when="+rocm amdgpu_target=%s" % arch
        )
        depends_on(
            "hipsolver+rocm amdgpu_target=%s" % arch, when="+rocm amdgpu_target=%s" % arch
        )
